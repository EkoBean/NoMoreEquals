...
只有會送進聊天頻道的行才會轉換。`/say`、`/p`、`/tell`、`/fc`、`/b`、`/l1`–`/l8`、`/cwl1`–`/cwl8`、`/em` 等聊天指令會轉換訊息本文，指令詞本身不動。其他指令（`/gearset`、`/target`、其他插件的指令）完全不介入，不轉換也不顯示預覽 —— 那些參數是識別碼不是訊息，換掉字形會讓指令失效。
